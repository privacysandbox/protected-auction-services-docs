...
*   `size`: unsigned integer indicating the number of bits based on the range of values.
      ...
      * For example, consider an `unsigned-integer-feature-type` of `size `= `3`. It can have a value: ` [0, 7] `and will occupy 3 bits on the wire.
...
*   `size`: unsigned integer indicating the number of bits based on the range of values.
    ...
    * For example, consider a `signed-integer-feature-type` of `size` = `4`. It can have a value `[-8, 7]`, and will occupy 4 bits on the wire.
